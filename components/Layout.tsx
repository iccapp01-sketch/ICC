...
import { Home, BookOpen, FileText, Music, Users, Video, Bell, User } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 transition-colors duration-300">
      ...
      <div className="fixed top-0 w-full z-50 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm h-16 flex items-center justify-between px-4 transition-colors duration-300">
        ...
        {/* Left: Welcome Text Only (Logo Removed) */}
        <div className="flex items-center gap-3">
           <div className="flex flex-col">
              <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">Good Morning</span>
              <span className="text-sm font-bold text-slate-900 dark:text-white leading-none">Welcome Back, {userName || 'Guest'}</span>
           </div>
        </div>
